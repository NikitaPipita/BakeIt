...
class DatabaseHelper {
  ...
  static final String _sqlGetCakeProducts =
      'SELECT '
  ...
  ///Add cake id at the end of SQL query
  static final String _sqlGetInformationAboutProductsAndTheirQuantityInTheCake =
      'SELECT '
  ...
  static Future<bool> copyDB() async {
    ...
    _path = join(databasesPath, "cakes.db");
  ...
  static Future<void> insertCake(String title) async {
    ...
    var response =
        await db.rawInsert('INSERT INTO cake(title) VALUES("$title")');
    print(response);
  }

  static Future<void> insertProduct(
      String title, double price, int count) async {
    final Database db = await _database;

    var response =
        await db.rawInsert('INSERT INTO product(title, price, count) '
            'VALUES("$title", $price, $count)');
    print(response);
  }

  static Future<void> insertCakeProduct(int productId, int productCount,
      {int cakeId}) async {
    final Database db = await _database;

    if (cakeId == null) {
      var maxCakeIdRequest =
          await db.rawQuery('SELECT MAX(_id) AS _id FROM cake');
      List<MaxCakeId> cakeIdAnswer =
          maxCakeIdRequest.map((e) => MaxCakeId.fromMap(e)).toList();
  ...
  static Future<List<CakeModel>> getCakes() async {
    ...
    List<CakeModel> cakeList =
        response.map((c) => CakeModel.fromMap(c)).toList();
    cakeList.sort();
    return cakeList;
  }
  ...
  static Future<List<ProductModel>> getProducts() async {
    ...
    List<ProductModel> productList =
        response.map((c) => ProductModel.fromMap(c)).toList();
    productList.sort();
    return productList;
  }
  ...
  static Future<List<IngredientModel>> getCakeProducts(int cakeId) async {
    ...
    var response = await db.rawQuery(_sqlGetCakeProducts + cakeId.toString());
    List<IngredientModel> ingredientList =
        response.map((c) => IngredientModel.fromMap(c)).toList();
    return ingredientList;
  }

  static Future<List<ProductInfoAndQuantityInCakeModel>>
      getProductsInfoAndTheirQuantityInCake(int cakeId) async {
    final Database db = await _database;

    var response = await db.rawQuery(
        _sqlGetInformationAboutProductsAndTheirQuantityInTheCake +
            cakeId.toString());
    List<ProductInfoAndQuantityInCakeModel> productInfoAndQuantityInCakeList =
        response
            .map((c) => ProductInfoAndQuantityInCakeModel.fromMap(c))
            .toList();
